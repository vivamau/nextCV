...
import { ArrowLeft, FileText, ExternalLink, Sparkles, Loader } from 'lucide-react';
import { useState } from 'react';
import { useTor, useTorSkills, extractTorSkills } from '../../hooks/useTors';
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h2 className="text-sm font-semibold text-gray-700 mb-3">
          Extracted Skills
          {!loadingSkills && <span className="ml-2 font-normal text-gray-400">({skills.length})</span>}
        </h2>
        ...
        {loadingSkills ? (
          ...
          <div className="flex flex-wrap gap-2">
            {skills.map(s => (
              <span key={s.id} className="px-2.5 py-1 bg-purple-50 text-purple-700 text-xs rounded-full border border-purple-100">
                {s.skill}
              </span>
            ))}
          </div>
